    ...
    while op != 0:
        if op == 1:
            ...
            while i<cantPlan:
                ...
                montoApagar=int(__listaPlanes[i].getCantidadCuotasLicitar())*importeCuota
            ...
            while ((i<cantPlan)and(bandera>=0)) :
                print('Código: ',__listaPlanes[i].getCodigo())
                print('Modelo: ',__listaPlanes[i].getModelo())
                print('Cuotas para Licitar: ',__listaPlanes[i].getCantidadCuotasLicitar())
                nuevoValor = input('Ingrese número de cuotas: ')
                if(int(nuevoValor)>0):
                    __listaPlanes[i].setCantidadCuotasLicitar(int(nuevoValor))
                    print('Se Modifico la cantidad de Cuotas para Licitar: ',__listaPlanes[i].getCantidadCuotasLicitar())
                    i=i+1
                else:
                    bandera=-1
        print('')           
